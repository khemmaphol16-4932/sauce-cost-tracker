// Mirrors src/lib/peripage.ts (its code with the types stripped) — keep in sync.
...
const PRINT_WIDTH = 384; // dots per line on the A6
...
function ditherToBlackAndWhite(canvas) {
  const ctx = canvas.getContext("2d");
  if (!ctx) return;
  const { width, height } = canvas;
...
function buildPrintBytes(rows, height) {
  const parts = [DENSITY_DARK];
...
async function writeToPrinter(port, parts) {
...
async function printCanvas(port, canvas) {
